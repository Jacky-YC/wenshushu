...
def download(url):
    def get_tid(token):
        r = s.post(url="https://www.wenshushu.cn/ap/task/token", json={"token": token})
...
def upload(filePath):
    ...
    def get_cipherheader(epochtime, token, data):
        ...
        key_iv = (
            ...
            "".join([epochtime[int(i)] for i in epochtime[::-1][:5]])
            + "000"
        ).encode()
    ...
    def storage():
        r = s.post(url="https://www.wenshushu.cn/ap/user/storage", json={})
        rsp = r.json()
    ...
    def userinfo():
        s.post(url="https://www.wenshushu.cn/ap/user/userinfo", json={"plat": "pcweb"})

    def getCode(tid):
        epochtime = get_epochtime()
        req_data = {"tid": tid, "is_create": True}
        # POST的内容在服务端会以字串形式接受然后直接拼接X-TOKEN，不会先反序列化JSON字串再拼接
        # 加密函数中的JSON序列化与此处的JSON序列化的字串形式两者必须完全一致，否则校验失败
        r = s.post(
            url="https://www.wenshushu.cn/ap/task/get_code",
            json=req_data,
            headers={
        ...
        data = rsp["data"]
        assert data, "需要滑动验证码"
        return data["code"], data["code_exp"]
    ...
    def psurl(fname, upId, file_size, partnu=None):
        ...
        r = s.post(url="https://www.wenshushu.cn/ap/uploadv2/psurl", json=payload)
        rsp = r.json()
    ...
    def copysend(boxid, taskid, preid):
        ...
        code, code_exp = getCode(rsp["data"]["tid"])
        print(f"临时取件码: {code}, 有效期: {code_exp} 秒")
    ...
    def fast():
        ...
        for _ in range(2):
            r = s.post(url="https://www.wenshushu.cn/ap/uploadv2/fast", json=payload)
            rsp = r.json()
        ...
        if ispart:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:  # or use os.cpu_count()
                ...
                for i in range((file_size + chunk_size - 1) // chunk_size):
                    ul_size = (
                        chunk_size
                        if chunk_size * (i + 1) <= file_size
                        else file_size % chunk_size
                    )
                    future_list.append(
                        executor.submit(
                            file_put,
                            [fname, upId, ul_size, i + 1],
                            filePath,
                            chunk_size * i,
                            ul_size,
                        )
...
if __name__ == "__main__":
    ...
    s.headers["Accept-Language"] = (
        "en-US, en;q=0.9"  # NOTE: require header, otherwise return {"code":-1, ...}
    )
    try:
        command = sys.argv[1]
